...
Список доступных каналов связи (channel):
${JSON.stringify(validChannels, null, 2)}

Список доступных стран (country):
${JSON.stringify(validCountries, null, 2)}
...
2. "client": выбери НАИБОЛЕЕ точное название клиента из списка доступных клиентов. Если не упоминается, подбери из выбранного магазина.
3. "storeId": если упоминается конкретный магазин (по имени, адресу или коду типа K201/C140), найди его ID в списке доступных магазинов и верни этот ID. Если упомянуты "все магазины", укажи "all".
4. "channel": выбери НАИБОЛЕЕ подходящее название канала связи из списка доступных каналов (например: Telegram, WhatsApp, Email, Телефон).
5. "country": выбери наименование страны из списка доступных стран (например: Казахстан, Россия, Узбекистан, Беларусь). Обязательно определи по названию магазина, города (Алматы/Астана -> Казахстан, Москва -> Россия, Ташкент -> Узбекистан, Минск -> Беларусь) или тексту.
6. "subject": сформулируй краткую и понятную тему обращения на русском языке (до 8-10 слов).
7. "description": подробно опиши проблему на русском языке так, как она изложена в аудио.
8. "system": определи подходящую систему из списка CategorySystem (BO, FO, MP, PC, HR, T&T).
9. "module": определи подходящий модуль из списка CategoryModule выбранной системы.
10. "type": определи подходящий тип из списка CategoryType выбранного модуля.
11. "action": определи подходящее действие из списка действий выбранного типа (если есть).
12. "kind": выбери наиболее подходящий вид обращения из списка доступных видов.
13. "datetime": дата и время обращения или возникновения проблемы (в формате "YYYY-MM-DDTHH:MM") относительно текущего времени сервера: ${currentDatetime}.
14. "startedWorkingAt": дата и время начала работы над тикетом (в формате "YYYY-MM-DDTHH:MM"), если упоминается начало работы.
15. "closedAt": дата и время закрытия/решения тикета (в формате "YYYY-MM-DDTHH:MM"), если проблема уже решена или закрыта.
16. "confirmedAt": дата и время подтверждения клиентом успешного закрытия тикета (в формате "YYYY-MM-DDTHH:MM"), если из аудио следует, что клиент подтвердил решение.
17. "resolutionComment": текст комментария решения проблемы, если звучит в аудио.
18. "status": статус обращения ("open", "in_progress", "resolved", "closed").

Верни ответ в строго соответствующем JSON-формате.`;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              startedWorkingAt: { type: Type.STRING },
              closedAt: { type: Type.STRING },
              confirmedAt: { type: Type.STRING },
              resolutionComment: { type: Type.STRING },
...
2. "client": выбери НАИБОЛЕЕ точное название клиента из списка доступных клиентов. Если не упоминается, подбери из выбранного магазина.
3. "storeId": если на скриншоте упоминается конкретный магазин (по имени, адресу или коду типа K201/C140), найди его ID в списке доступных магазинов и верни этот ID. Если упомянуты "все магазины", укажи "all". 
4. "channel": выбери НАИБОЛЕЕ подходящее название канала связи из списка доступных каналов (например: Telegram, WhatsApp, Email, Телефон).
5. "country": выбери наименование страны из списка доступных стран (например: Казахстан, Россия, Узбекистан, Беларусь). Обязательно определи по названию магазина, города (Алматы/Астана -> Казахстан, Москва -> Россия, Ташкент -> Узбекистан, Минск -> Беларусь) или тексту на скриншоте.
6. "subject": сформулируй краткую и понятную тему обращения на русском языке (до 8-10 слов) на основе ошибки или проблемы.
7. "description": подробно опиши проблему на русском языке так, как она видна на изображении (текст ошибки, контекст, детали экрана).
8. "system": определи подходящую систему из списка CategorySystem (BO, FO, MP, PC, HR, T&T).
9. "module": определи подходящий модуль из списка CategoryModule выбранной системы.
10. "type": определи подходящий тип из списка CategoryType выбранного модуля.
11. "action": определи подходящее действие из списка действий выбранного типа (если есть).
12. "kind": выбери наиболее подходящий вид обращения из списка доступных видов.
13. "datetime": дата и время обращения или возникновения проблемы (в формате "YYYY-MM-DDTHH:MM") относительно текущего времени сервера: ${currentDatetime}.
14. "startedWorkingAt": дата и время начала работы над тикетом (в формате "YYYY-MM-DDTHH:MM"), если на скриншоте есть отметка времени взятия в работу.
15. "closedAt": дата и время закрытия/решения тикета (в формате "YYYY-MM-DDTHH:MM"), если на скриншоте видно, что проблема решена или тикет закрыт.
16. "confirmedAt": дата и время подтверждения клиентом успешного решения (в формате "YYYY-MM-DDTHH:MM"), если в чате видно сообщение с подтверждением от клиента.
17. "resolutionComment": комментарий решения тикета (текст ответа поддержки или описание решения), если виден на скриншоте.
18. "status": статус обращения ("open", "in_progress", "resolved", "closed").
